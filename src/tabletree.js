(function () {
  angular
    .module('template/tabletree.html', [])
    .run([
      '$templateCache',
      function ($templateCache) {
        $templateCache.put('template/tabletree.html',
  ...
  angular
    .module('atmira.ui.tabletree', [
      'template/tabletree.html'
      //'template/tabletree/tabletree.html'
    ])
      ...
      function ($timeout,
        ...
        return {
          ...
          templateUrl: function (tElement, tAttrs) {
            return tAttrs.templateUrl || tabletreeTemplate.getPath();
          },
          replace: true,
          scope: {
            treeData: '=',
          ...
          link: function (scope, element, attrs) {
            ...
            attrs.iconExpand = attrs.iconExpand ? attrs.iconExpand : 'iconPlus';
            attrs.iconCollapse = attrs.iconCollapse ? attrs.iconCollapse : 'iconMinus';
            // attrs.sortedAsc = attrs.sortedAsc ? attrs.sortedAsc : 'fa angle-up';
            // attrs.sortedDesc = attrs.sortedDesc ? attrs.sortedDesc : 'fa angle-down';
            attrs.expandLevel = attrs.expandLevel ? attrs.expandLevel : '0';
            expand_level = parseInt(attrs.expandLevel, 0);

            if (!scope.treeData) {
              alert('No data was defined for the tree, please define data to show! <3');
              return;
            }
    ...
    .provider('tabletreeTemplate', function () {
      var templatePath = 'template/tabletree.html';
    ...
    .filter('searchFor', function () {
      return function (arr, filterString, expandingProperty, colDefinitions, expand) {
        ...
        for (var i = 0; i < filtered.length; i++) {
          if (!filtered[i].branch.hasOwnProperty('par')) {
            if (i % 2 == 0) {
              // es par
              filtered[i].branch.par = true;
              for (var j = 0; j < filtered[i].branch.children.length; j++) {
                filtered[i].branch.children[j].par = true;
              }
            } else {
              // es impar
              filtered[i].branch.par = false;
              for (var x = 0; x < filtered[i].branch.children.length; x++) {
                filtered[i].branch.children[x].par = false;
              }
            }
          }
        }
